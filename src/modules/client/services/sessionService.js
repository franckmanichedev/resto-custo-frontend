...
class SessionManager {
    ...
    setSession(sessionData) {
        ...
        if (tableNumber) {
            tableNumber.textContent = sessionData.table?.number || sessionData.table?.name || '-';
        }

        if (sessionData.session?.session_extended) {
            showToast('Votre session a ete prolongee car une commande est en cours', 'info');
        }
