...
async function loadCompositions() {
    const response = await api.compositions.getAll();
...
async function loadCategories(kind, selectedId = '') {
    const response = await api.categories.getAll({ kind });
...
async function loadTypeCategories(categoryId, selectedId = '') {
    ...
    const response = await api.categories.getTypes(categoryId);
...
async function ensureCategorySelection() {
    ...
    const response = await api.categories.create({ name: newCategoryName, kind });
...
async function ensureTypeCategorySelection(categoryId) {
    ...
    const response = await api.categories.createType({ categorie_id: categoryId, name: newTypeName });
...
async function loadItems() {
    try {
        ...
        const response = await api.plats.getAll(params);
...
async function editItem(id) {
    try {
        const response = await api.plats.getById(id);
...
async function toggleItem(id) {
    try {
        await api.plats.toggleAvailability(id);
...
itemForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (editingItemId) {
            await api.plats.update(editingItemId, formData);
            showToast('Article modifie avec succes', 'success');
        } else {
            await api.plats.create(formData);
